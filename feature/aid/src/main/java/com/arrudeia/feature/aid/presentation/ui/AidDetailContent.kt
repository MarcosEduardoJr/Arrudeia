...
import com.arrudeia.feature.aid.R.string.step_to_step
...
@Composable
private fun contentDescription(item: AidDetailUiModel?) {
    LazyColumn(
        modifier = Modifier
            .padding(16.dp)
            .fillMaxSize()
...
@Composable
private fun include(item: AidDetailUiModel?) {
    if (item?.steps.orEmpty().isNotEmpty()) {
        Text(
            text = stringResource(step_to_step),
            color = Color.Black,
            fontSize = 16.sp,
            fontWeight = FontWeight.Bold,
            textAlign = TextAlign.Start
